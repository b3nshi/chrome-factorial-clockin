...
const autofill = function () {
  ...
  const autofillExecution = (arrayOfTimes) => {
      ...
      Array.from(tableRows).forEach((tr) => {
        if (tr.className.indexOf('disabled') === -1) {
          ...
          try {
            ...
            logger(
              'it was not found an special label to indicate it is a holiday'
            );
          }

          if (!isVacation && !['s', 'd', 'S', 'D'].includes(day[0])) {
